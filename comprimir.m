...
% el nº píxeles truncados total es ceil(sqrt((s - s / r) / (n/k)^2))^2 * (n/k)^2  donde r es el ratio de compresión, s es el
...
function [M2] = comprimir(M, r, k)
	...
	for j=1:d
		...
		C = blockproc(C, [k,k], @(X) truncate(X, repmat(ceil(sqrt((s-s/r)/(n/k)^2)), 1, 2), 0));
